...
because $c_{11}$, $c_{33}$ and $c_{55}$ are positive definite and $\theta$ is defined from
$0^{\circ}$ (vertical) to $90^{\circ}$ (horizontal). 
...
Case 1 is considered the "normal" case. Apart from the odd polarizations for this 
scenario, as mentioned above, the slowness surfaces are elliptical.
...
In this case we can't say whether $V_a$ or $V_b$ is larger for all values of $\theta$ 
since both $c_{11}$ and $c_{33}$ are greater than $c_{55}$. However, it is clear
for small values of $\theta$ $V_b$ will be larger and for larger values of $\theta$
(near $90^{\circ}$) $V_{a}$ will be larger.
